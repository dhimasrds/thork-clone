    ...
    private fun validationDateAndTime(){
        if (validationEmpty()) {
                if (msEndTime!! < msStartTime!!) {
                    Toast.makeText(this@CreateLaborActualActivity, "Finish time has to be after Start time ", Toast.LENGTH_SHORT).show()
                }
    ...
    private fun validationEmpty():Boolean{
        binding.apply {
            if (tvLabor.text.isNullOrBlank() ||
                ...
                Toast.makeText(this@CreateLaborActualActivity, "field cannot be empty", Toast.LENGTH_SHORT).show()
            return false
            }
        }
        return  true
